...
def build_dora_backbone(lora_r=16, lora_alpha=32, lora_dropout=0.05):
    ...
    peft_model.gradient_checkpointing_enable()  # will slow when enable
...
def make_dataset(sequences, labels, groups, sections, batch_size=8):
    ...
    # outer split: ~15% test, group-disjoint, label-stratified
    train_val_idx, test_idx = _stratified_group_split(
        indices, labels_a, groups_a, test_frac=0.2, seed=SEED,
    )

    # inner split: take ~15/85 of the inner pool as val
...
def hyperparameters_queryer(*args):
    {'lora_r': 4, 'lora_alpha': 32, 'lora_dropout': 0.1, 'num_heads': 8, 'dropout': 0.1, 'pooling_mode': 'attention', 'batch_size': 16, 'max_epoch': 50, 'weight_fold': 0.1, 'max_weight': 100, 'lr': 0.0001, 'left_window': 411, 'right_window': 101, 'ignore_sec': -1}
    hp = {
        "lora_r":       4, #4,
        "lora_alpha":   32,
        "lora_dropout": 0.1,
