...
def get_criterion_fn(hyperparameters: dict, neg_pos_ratio: float):
    import torch
    import torch.nn.functional as F

    pos_weight = neg_pos_ratio * float(hyperparameters.get("pos_weight_fold", 1.0))
    ...
    def criterion_fn(logits, labels, sections):
        ...
        sample_w[sections != 0] = hyperparameters.get('non_annotated_weight_fold', 1)
...
def train_model(train_loader, val_loader, hp: dict, neg_pos_ratio,
                patience=15):

    print("hyperparameters:", hp)
    model = build_model(hp, DEVICE)

    criterion_fn = get_criterion_fn(hp, neg_pos_ratio)
...
def make_dataset(sequences, labels, groups, sections, batch_size=8):
    ...
    neg_pos_ratio = label_counts[0] / label_counts[1]
    ...
    print("train sections:", dict(Counter(train_sections)))
    return (
...
def hyperparameters_queryer(*args):
    hp = {
        ...
        "batch_size":   16,
        "max_epoch":    50,
        'non_annotated_weight_fold': 10,
        "pos_weight_fold": 0.5,
        "max_weight": 200,
        "lr":           1e-4,
        "left_window":  410,    # RiNALMo trained up to ~1024; safe upper limit
        "right_window": 100,
...
def main():
    ...
    if len(sys.argv) > 1 and sys.argv[1] == 'train':  # train mode
        train_model(
            train_loader, val_loader, hp, neg_pos_ratio, patience=5,
        )

    model = load_model(SAVE_PATH, hp, DEVICE)

    print('========== non-indicated sites ==========')
