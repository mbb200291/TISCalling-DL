...
class RiNALMoClassifier(nn.Module):
    ...
    def forward(
        ...
        lengths:        torch.Tensor = None,   # [B]  (unused, kept for API compat)
...
def centralize_transcript(left_window: int, right_window: int, seq: str, position: int):
    seq_left_preserve = 99
    seq_right_preserve = 102
    # preserve at most 99 nt before marker and 102 nt after marker
    preserve_lb = max(0, position - seq_left_preserve)
    preserve_rb = min(len(seq), position + seq_right_preserve + 1)
    seq = seq[preserve_lb:preserve_rb]
    # marker position after trimming
    position = position - preserve_lb
...
def main():
    ...
    # df = pd.read_csv("df_test.csv").reset_index(drop=True)
    ...
    model = load_model('model_1k.pt', hp, DEVICE)
    ...
    df_test['yhat_prob_giga1k_201truncate'] = yhat_prob.numpy()
    df_test['yhat_giga1k_201truncate'] = yhat.numpy()
    df_test.to_csv('df_test_add_giga1k_201truncated.csv')
