...
DEVICE = torch.device("cuda:1" if torch.cuda.is_available() else "cpu")  # temp
print('DEVICE:', DEVICE)
# RNA vocabulary
VOCAB = {
    "A": 1,
...
def collate_fn(batch):
    ...
    # +1 for [CLS]
    padded_seqs = torch.full((batch_size, 1 + max_len), PAD_IDX, dtype=torch.long)
    padded_feats = torch.zeros((batch_size, 1 + max_len), dtype=torch.float)

    for i, (seq, feat) in enumerate(zip(seqs, feats)):
        seq_len = len(seq)
        padded_seqs[i, 0] = CLS_IDX          # [CLS] token
        padded_seqs[i, 1:1 + seq_len] = seq
        # padded_feats[i, 0] stays 0.0 for [CLS]
        padded_feats[i, 1:1 + seq_len] = feat
...
class RNASequenceBinaryClassifier(nn.Module):
    def __init__(
        ...
        # vocab_size must cover CLS_IDX; caller should pass max(VOCAB.values())+2
        ...
        # +1 offset because CLS is prepended (position ids shift by 1)
        self.position_embedding = nn.Embedding(max_len * 2 + 1, d_model)
        self.frame_embedding = nn.Embedding(math.ceil(2 * max_len / 3) + 2, d_model)
        ...
        # ALL three modes: concat with [CLS] → 2*D input
        # marker  : [marker_h ; cls_h]
        # mean    : [mean_h   ; cls_h]   (was [marker_h ; mean_h], now adds cls)
        # attention: [attn_h  ; cls_h]   (was [marker_h ; attn_h], now adds cls)
        # ── if you want to keep marker_h in mean/attention, bump to 3*D ──
    ...
    def forward(
        self,
        input_ids: torch.Tensor,      # [B, 1+L]  (CLS prepended)
        pos_features: torch.Tensor,   # [B, 1+L]
        lengths: torch.Tensor,        # [B]  original seq lengths (no CLS)
    ) -> torch.Tensor:
        del lengths

        B, total_L = input_ids.shape  # total_L = 1 + L

        if total_L - 1 > self.max_len:
            raise ValueError(
                f"Sequence length {total_L - 1} exceeds max_len={self.max_len}"
            )

        # embeddings
        seq_emb = self.embedding(input_ids)            # [B, 1+L, E]
        ind = pos_features.unsqueeze(-1).float()       # [B, 1+L, 1]
        x = torch.cat([seq_emb, ind], dim=-1)          # [B, 1+L, E+1]
        x = self.input_proj(x)                         # [B, 1+L, D]
        x = self.input_norm(x)

        # positional / frame embeddings
        # marker_pos is at index (original_pos + 1) because of CLS prefix
        # pos_features[:, 0] == 0 (CLS slot), argmax returns first 1 in real seq
        marker_pos = pos_features[:, 1:].argmax(dim=1) + 1  # [B]  (1-indexed in full tensor)

        pos_ids = torch.arange(total_L, device=input_ids.device).unsqueeze(0)  # [1, 1+L]
        # relative position: CLS gets  (0 - marker_pos + max_len - 1)
        relative_pos_id = pos_ids - marker_pos.unsqueeze(1) + self.max_len  # [B, 1+L]
        ...
        # padding mask
        padding_mask = input_ids.eq(self.pad_idx)   # [B, 1+L]
        padding_mask[:, 0] = False                   # ensure CLS is not masked

        # transformer encoder
        out = self.encoder(x, src_key_padding_mask=padding_mask)  # [B, 1+L, D]

        # extract representations
        cls_h = out[:, 0, :]                                      # [B, D]  ← [CLS]

        batch_idx = torch.arange(B, device=input_ids.device)
        marker_h = out[batch_idx, marker_pos, :]                  # [B, D]

        if self.pooling_mode == "marker":
            # [marker_h ; cls_h]
            h = torch.cat([marker_h, cls_h], dim=-1)              # [B, 2D]

        elif self.pooling_mode == "mean":
            # mean over real tokens (exclude CLS to keep semantics clean),
            # then concat cls_h
            mean_h = self._masked_mean_pool(
                out[:, 1:, :], padding_mask[:, 1:]
            )                                                      # [B, D]
            h = torch.cat([mean_h, cls_h], dim=-1)                # [B, 2D]

        elif self.pooling_mode == "attention":
            # marker-guided attention over real tokens, then concat cls_h
            ...
            h = torch.cat([global_h, cls_h], dim=-1)              # [B, 2D]
...
def train_model(
    ...
    model = RNASequenceBinaryClassifier(
        vocab_size=max(VOCAB.values()) + 2, # one for PAD, one for CLS
        **hp_queryer(
...
def hyperparameters_queryer(*args):
    return {k: v for k, v in {
        'embed_dim': 32,
        'd_model': 64, #128,#64, # 32 ~ 64
        'num_layers': 2, #3, # 1 ~ 2
        'num_heads': 8, #16, # 2 ~ 4 
        'ff_dim': 128, #64 ~ 128 (convension = d_model * 2)
        'batch_size': 32, # 32, #16, # 32 ~ 64
        'max_epoch': 50,
        'dropout': 0.3, #0.2, 
        'weight_fold': 0.1 , #0.5, #1,
        'max_len': 511, #3000, #2047, #4096,
        'pooling_mode': 'mean',
        'lr': 1e-3,
