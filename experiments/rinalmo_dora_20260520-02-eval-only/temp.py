...
def _stratified_group_split(
    ...
    n_splits = max(2, round(1.0 / float(test_frac)))
    ...
    train_idx, test_idx = next(sgkf.split(indices, labels, groups=groups))
    return train_idx, test_idx
...
def make_dataset(sequences, labels, groups, sections, batch_size=8):
    ...
    # gss_val = GroupShuffleSplit(n_splits=1, test_size=0.1/0.8, random_state=SEED)
    ...
    train_val_rel, test_idx = _stratified_group_split(
        indices=indices,
        labels=labels_a,
        groups=groups_a,
        test_frac=0.2,
    ...
    inner_indices = indices[train_val_rel]
    inner_labels = labels_a[train_val_rel]
    inner_groups = groups_a[train_val_rel]
    ...
    train_idx = train_val_rel[train_rel]
    val_idx = train_val_rel[val_rel]
...
def main():
    ...
    df = pd.read_csv("../../input_data/df_data.csv")
    train_loader, val_loader, test_loader, pos_weight, train_idx, val_idx, test_idx = get_dataset(df, hp)
    if len(sys.argv) > 1 and sys.argv[1] != 'eval':  # train mode
