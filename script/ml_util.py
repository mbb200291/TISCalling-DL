...
def group_aware_train_test_split(X_df, y, groups, test_size=0.2, random_state=42):
    ...
    return sgkf.split(X_df, y, groups=groups)
...
def ml_training_and_testing(X_df, y, groups, model_name, model_dir):
    ...
    def single_run_ml_training_and_testing(train_idx, test_idx, round_num):
        X_train = X_df.iloc[train_idx]
        X_test  = X_df.iloc[test_idx]
        y_train = y[train_idx]
        y_test  = y[test_idx]
        ...
        for name, clf in classifiers.items():
            ...
            def random_downsample_avg():    
                ...
                return [f1_test_down, aucroc_test_down, mcc_test_down, accuracy_test_down, recall_test_down, precision_test_down]

            test_down_sampled = []
            for _ in range(30):
                test_down_sampled.append(random_downsample_avg())
            f1_test_down, aucroc_test_down, mcc_test_down, accuracy_test_down, recall_test_down, precision_test_down = map(
                lambda x: sum(x) / len(x),
                zip(*test_down_sampled)
            )
    ...
    outer_cv_results = []
    for i, (train_idx, test_idx) in enumerate(group_aware_train_test_split(X_df, y, groups, test_size=0.2, random_state=42)):
        outer_cv_results.extend(single_run_ml_training_and_testing(train_idx, test_idx, i))

    
    out_summary = f'{out}/Model_evaluation_results.tsv'
    results_df = pd.DataFrame(outer_cv_results)
